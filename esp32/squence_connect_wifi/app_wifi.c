...
extern protocol_type protocol_using;
extern protocol_type last_protocol;
                          ...
                          int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        s_retry_num = 0;
        wifi_started = true;
        last_protocol = protocol_using;
        protocol_using = WIFI_PROTOCOL;
        xEventGroupSetBits(s_wifi_event_group, WIFI_CONNECTED_BIT);
    }
    else if (event_base == IP_EVENT && event_id == WIFI_EVENT_DISCONNECT_MANUAL)
    {
        ESP_LOGI(TAG, "WIFI_EVENT_DISCONNECT_MANUAL");
    }
}
...
esp_netif_t* esp_netif_create_wifi_sta(void)
{
    ...
    netif_wifi_config.route_prio = 0;
    netif_wifi_config.if_desc = "netif_wifi";
    netif_wifi_config.if_key = "netif_wifi";
...
void wifi_init_sta(char *ssid, char* password)
{
    ...
    if (init_sta == false)
    {
        ...
        wifi_netif = esp_netif_create_wifi_sta();
    ...
    if (init_sta)
    {
        esp_wifi_disconnect();
        // esp_wifi_stop();
        // ESP_LOGW (TAG, "WIFI STOP NOW");
        // ESP_ERROR_CHECK(esp_wifi_start());
        s_retry_num = 0;
        esp_wifi_connect();
        ESP_LOGI(TAG, "wifi_init_sta finished, ssid %s, password %s conect again", ssid, password);
    }
    else
    {
